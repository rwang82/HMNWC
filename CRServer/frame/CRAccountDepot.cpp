...
bool CRAccountDepot::addAccount( CRAccountBase* pAccountNew, int& nErrCode ) {
	...
	if ( _hasAccount( pAccountNew->m_data.m_tstrUserName, nErrCode ) )
		return false;
	//
	if ( !g_CRSrvRoot.m_pSrvDBProxy->save2DB( pAccountNew, nErrCode ) )
		return false;
	m_mapName2AccountObj[ pAccountNew->m_data.m_tstrUserName ] = pAccountNew;
	//
	return true;
}
...
bool CRAccountDepot::_loadAccountFromDB( const tstr_container_type& containerAccountName, int& nErrCode ) {
	...
	for ( itAccount = accountList.m_containerAccount.begin(); itAccount!=iendAccount; ++itAccount ) {
		...
		itName2Obj = m_mapName2AccountObj.find( pAccountObjNew->m_data.m_tstrUserName );
	    if ( itName2Obj != m_mapName2AccountObj.end() ) {
		    itName2Obj->second->m_data = pAccountObjNew->m_data;
			delete pAccountObjNew;
			pAccountObjNew = NULL;
		} else {
		    m_mapName2AccountObj[ pAccountObjNew->m_data.m_tstrUserName ] = pAccountObjNew;
		}
	}
